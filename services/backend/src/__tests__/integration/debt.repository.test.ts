...
describe('DebtRepository', () => {
    ...
    describe('getAll', () => {
        ...
        it('should return debts generated by sales assigned to a close', async () => {
            const customer = await createTestCustomer()
            const sale1 = await createTestSale()
            const sale2 = await createTestSale()
            const debt1 = await createTestDebt(sale1.id, customer.id)
            await createTestDebt(sale2.id, customer.id)
            const close = await createTestClose()

            await saleRepository.setClosed(close.id, [sale1.id])

            const debts = await debtRepository.getAll({ close_id: close.id })

            expect(debts).toHaveLength(1)
            expect(debts[0].id).toBe(debt1.id)
        })
    })
